...
def foodHeuristic(state, problem):
    ...
    #euristica aleasa: distanta manhattan pana la cea mai apropiata mancare + distanta
    #manhattan pana la ea pana la cea mai indepartata mancare de ea  - consistente
    heuristic = 0
    foodCoordinates = foodGrid.asList() #lista cu coordonatele mancarii
    if len(foodCoordinates) == 0: #daca nu mai e mancare returnez 0
        return 0

    #doresc sa calculez distanta pana la cea mai apropiata mancare
    closestDistance = None
    posNearestFood = None

    for foodPos in foodCoordinates:
        currentDistance = util.manhattanDistance(position, foodPos)
        if (closestDistance == None or currentDistance < closestDistance) and currentDistance != 0:
            closestDistance = currentDistance
            posNearestFood = foodPos
    #actualizez pozitia pe pozitia celei mai apropiate bucati de mancare
    #aflu distanta manhattan pana la cea mai apropiata mancare si o adaug la euristica
    heuristic += closestDistance

    farthestDistance = None
    for foodPos in foodCoordinates:
        currentDistance = util.manhattanDistance(posNearestFood, foodPos)
        if (farthestDistance == None or currentDistance > farthestDistance):
            farthestDistance = currentDistance

    # aflu distanta manhattan pana la cea mai indepartata mancare (de la mancarea precedenta) si o adaug la euristica
    heuristic += farthestDistance
